defmodule Yacht do
  ...
  def score(category, dice = [f, _, _, _, _]) when
    category in @categories_requiring_sorting and
      f in 1..6,
  ...
  defp score_sorted(:full_house, [d1, d1, d2, d2, d2]) when
      d1 != d2,
    do: 2 * d1 + 3 * d2
  ...
  defp score_sorted(_, _), do: 0

  @spec sum_of_face(face(), [face()]) :: points()
  defp sum_of_face(face, dice) do
    dice
    |> Enum.count(&(&1 == face))
    |> Kernel.*(face)
  end
