defmodule Say do
  ...
  for {divisor, name} <- [{1_000_000_000, "billion"}, {1_000_000, "million"}, {1_000, "thousand"}] do
    defp say(n, acc) when n >= unquote(divisor),
      do: say(rem(n, unquote(divisor)), [english_number(div(n, unquote(divisor))) <> unquote(" " <> name) | acc])
  end

  defp say(0, acc), do: acc |> Enum.reverse() |> Enum.join(" ")
  defp say(n, acc), do: say(0, [english_number(n) | acc])

  defp english_number(n) when n >= 100 and rem(n, 100) == 0,
  ...
  defp english_number(n) when n >= 20,
