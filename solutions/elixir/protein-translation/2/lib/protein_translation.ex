defmodule ProteinTranslation do
  ...
  defp acids(<<codon::binary-size(3), rest::binary>>, proteins),
    do: acids(of_codon(codon), rest, proteins)
  defp acids(_, _proteins),                        do: {:error, "invalid RNA"}
  defp acids({:ok, "STOP"},  _rest, proteins),     do: {:ok, Enum.reverse(proteins)}
  defp acids({:ok, protein},  rest, proteins),     do: acids(rest, [protein | proteins])
  defp acids({:error, _},    _rest, _proteins),    do: {:error, "invalid RNA"}
