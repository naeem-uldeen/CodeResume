defmodule ListOps do
  ...
  def count(list), do: foldl(list, 0, fn _head, acc -> acc + 1 end)

  def reverse(list), do: foldl(list, [], fn head, acc -> [head | acc] end)
  ...
  def filter(list, fun) do
    list
    |> foldl([], fn head, acc -> if fun.(head), do: [head | acc], else: acc end)
    |> reverse()
  end
  ...
  def concat(list) do
    list
    |> reverse()
    |> foldl([], fn head, acc -> append(head, acc) end)
  end
