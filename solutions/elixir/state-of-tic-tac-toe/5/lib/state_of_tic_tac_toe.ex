defmodule StateOfTicTacToe do
  ...
  ## Examples
      ...
      iex> StateOfTicTacToe.game_state("XXX\\nOO.\\n...\\n")
      {:ok, :win}
  ...
  def game_state(
    ...
    with {x_count, o_count} = count_marks(board, 0, 0),
         x_wins = win?(board, ?X),
         o_wins = win?(board, ?O),
         true <- x_count <= o_count + 1 || @wrong_turn_x_twice,
  ...
  defp count_marks(<<?X, rest::binary>>, x_count, o_count), do: count_marks(rest, x_count + 1, o_count)
  defp count_marks(<<?O, rest::binary>>, x_count, o_count), do: count_marks(rest, x_count, o_count + 1)
  defp count_marks(<<c, rest::binary>>, x_count, o_count) when c in [?., ?\n], do: count_marks(rest, x_count, o_count)
  
  defp win?(board, mark),do: Enum.any?(@lines, fn line -> Enum.all?(line, &(mark_at(board, &1) == mark)) end)
  
  defp mark_at(board, {row, col}), do: :binary.at(board, @offset_per_row * row + col)
