...
transform m = fromList (transformList (toList m) [])

transformList :: [(Int, String)] -> [(Char, Int)] -> [(Char, Int)]
transformList [] acc = acc
transformList ((score, cs) : xs) acc = transformList xs (transformLetters cs score acc)

transformLetters :: String -> Int -> [(Char, Int)] -> [(Char, Int)]
transformLetters [] _ acc = acc
transformLetters (c : cs) score acc = transformLetters cs score ((toLower c, score) : acc)
