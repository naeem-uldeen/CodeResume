class PhoneNumber
  ...
  def initialize(phone_number)
    ...
    in 11 if digits.start_with?('1')
      digits
    in 11
      raise PhoneNumberErrors::CountryCode, "invalid country code #{digits.chr}"
    else
    ...
    raise PhoneNumberErrors::AreaCode, "area code cannot start with #{area.chr}" unless
      area.chr =~ VALID_CODE_START
    raise PhoneNumberErrors::Exchange, "exchange code cannot start with #{exchange.chr}" unless
      exchange.chr =~ VALID_CODE_START
  end
