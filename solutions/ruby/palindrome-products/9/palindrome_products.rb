class Palindromes
  ...
  RANGE_COMPARASION_STRATEGIES = {
    smallest: [->(from, to) { lo, hi = [from, to].sort; lo.send(:upto, hi)   }, proc(&:<)],
    largest:  [->(from, to) { lo, hi = [from, to].sort; hi.send(:downto, lo) }, proc(&:>)]
  }
  private_constant :RANGE_COMPARASION_STRATEGIES
  ...
  def find_extreme order
    make_range, better = RANGE_COMPARASION_STRATEGIES[order]
