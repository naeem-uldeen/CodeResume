class Palindromes
  ...
  STRATEGIES = {
    smallest: [->(from, to) { from.upto(to) },   ->(candidate, incumbent) { candidate < incumbent }],
    largest:  [->(from, to) { to.downto(from) }, ->(candidate, incumbent) { candidate > incumbent }]
  }
  ...
  def find_extreme order
    make_range, better = STRATEGIES[order]
