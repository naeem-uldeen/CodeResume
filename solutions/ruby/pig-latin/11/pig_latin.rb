...
module PigWord
  ...
  def self.from(word)
    case
    when VOWELS.include?(word.chr) then Vowel.new word: word
    when UNUSUAL_PAIRS.any? { |pair| word.start_with?(pair) }
      then UnusualPair.new word: word
  ...
  module Consonant
    ...
    def self.from word
      ...
      case
        when q_index && word[q_index.next] == 'u' then QuCluster.from word
        when word.chars.drop(1).include?('y')     then YVowel.from word
